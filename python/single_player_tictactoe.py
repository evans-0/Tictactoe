...
def comp_mark(sym):
    ...
    for r, c in avail:
        l1[r][c] = sym
        if isWin():  #Checking if game won after computer marked
            l[r][c] = sym
            break
        else:
            l1[r][c] = '-'
    else:
        n = random.randint(0, len(avail))
...
def play(size):
    """If player won, then returns 1. If computer won, then returns 2. If game tied, then returns 0."""
    create(size)
    i = 0
